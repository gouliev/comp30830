...
function initMap() {
  ...
  const url =
    "https://api.jcdecaux.com/vls/v1/stations?contract=dublin&apiKey=f9e09d85362311d1646d868797cbc7100ded0c8d";
  ...
  function addMarker(bikeData) {
    ...
    if (bikeIconSelect >= 0.4) {
      bike_icon = "http://maps.google.com/mapfiles/ms/icons/green-dot.png";
    } else if (bikeIconSelect > 0.2 && bikeIconSelect < 0.4) {
    ...
    function calcRoute() {
      ...
      directionsService.route(request, (result, status) => {
        if (status == google.maps.DirectionsStatus.OK) {
          ...
          //error message
          output.innerHTML =
            "<div class = 'alert-danger'> Could not retrieve walking distance. </div>";
        }
      });
    }
    calcRoute();
